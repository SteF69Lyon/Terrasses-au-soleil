...
## 🌅 Reprise demain — à faire en premier

**Contexte** : journée du 2026-04-22 = exécution complète du plan SEO (Tasks 1-23) + scoring déterministe (Phases 1+2) + CI/CD via GitHub Actions + enregistrement Search Console. Une 20aine de PRs mergées.

**À faire demain matin (5 min) :**

1. **Vérifier l'état de la prod** : ouvrir https://terrasse-au-soleil.fr — la landing doit s'afficher, et `https://terrasse-au-soleil.fr/terrasses/paris/marais/` doit retourner une page complète avec cartes d'établissements. Si 404 → voir point 3.

2. **Vérifier que le dernier GHA run est passé au vert** :
   ```bash
   gh run list --workflow=deploy.yml --limit 3
   ```
   Le run `24790002010` (fix flat polygons, commit `3f8624`) doit être `completed success`. Si ce run a échoué, lire les logs avec `gh run view <id> --log | grep -iE "error|fail"`.

3. **Re-soumettre le sitemap dans Search Console** : https://search.google.com/search-console → propriété terrasse-au-soleil.fr → Sitemaps → cliquer sur le sitemap existant → **Actualiser**. Le sitemap doit maintenant contenir les 170 URLs (au lieu de 1 actuellement, parce que le dernier build complet n'avait pas encore tout uploadé).

4. **Flat polygon fix à valider** : les pages `/terrasses/[ville]/[quartier]/` doivent afficher des scores et des analyses sun pour chaque établissement, avec indicateur "Ombre portée" quand pertinent.
